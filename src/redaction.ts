...
function collectExactSecrets(
	...
	for (const secret of config.additionalSecrets ?? []) {
		add(secret, "configured-secret");
	}
	for (const [key, value] of Object.entries(env)) {
		if (key === "PI_SHARE_ADDITIONAL_SECRETS") continue;
...
export function redactString(
	...
	if (!config.enabled || !input) return input;
	let output = input;
...
function redactionMetadata(config: RedactionConfig) {
	return { applied: config.enabled };
}
